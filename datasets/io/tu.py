...
def read_tu_data(folder, prefix):
    ...
    data = Batch(x=x, edge_index=edge_index, edge_attr=edge_attr, y=y)
    data, slices = split(data, batch)

    data.batch_level += 1

    data.batch0 = batch
    data.inc_dict['batch0'] = data.num_graphs
    data.cat_dim_dict['batch0'] = 0

    data.ptr0 = slices['x'][:-1]
    data.inc_dict['ptr0'] = data.num_nodes
    data.cat_dim_dict['ptr0'] = 0

    data.edge_slice0 = slices['edge_index'][:-1]
    data.inc_dict['edge_slice0'] = data.num_edges
    data.cat_dim_dict['edge_slice0'] = 0
