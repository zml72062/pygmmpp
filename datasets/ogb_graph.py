...
from .io.ogb import (read_csv_graph_raw, 
...
class OGBG(Dataset):
    def __init__(self, name, root='dataset', transform=None, pre_transform=None, meta_dict=None):
        ...
        if meta_dict is None:
            ...
            master = pd.read_csv(os.path.join(
                os.path.dirname(__file__), 'master.csv'), index_col=0)
    ...
    def process(self):
        ...
        if self.task_type == 'subtoken prediction':
            graph_label_notparsed = pd.read_csv(osp.join(
                self.raw_dir, 'graph-label.csv.gz'), compression='gzip', header=None).values
            ...
            if self.binary:
                ...
                graph_label = pd.read_csv(osp.join(
                    self.raw_dir, 'graph-label.csv.gz'), compression='gzip', header=None).values
            ...
            for i, g in enumerate(data_list):
                if 'classification' in self.task_type:
                    if has_nan:
                        g.__set_tensor_attr__('y', torch.from_numpy(graph_label[i]).view(
                            1, -1).to(torch.float32), 'graph_feature')
                    else:
                        g.__set_tensor_attr__('y', torch.from_numpy(graph_label[i]).view(
                            1, -1).to(torch.long), 'graph_feature')
                else:
                    g.__set_tensor_attr__('y', torch.from_numpy(graph_label[i]).view(
                        1, -1).to(torch.float32), 'graph_feature')
...
def read_graph_pyg(raw_dir, add_inverse_edge = False, additional_node_files = [], additional_edge_files = [], binary = False) -> List[Data]:
    if binary:
        # npz
    ...
    for graph in tqdm(graph_list):
        g = Data()
        g.__set_tensor_attr__('edge_index', torch.from_numpy(graph['edge_index']),
                              'edge_index')
        ...
        if graph['edge_feat'] is not None:
            g.__set_tensor_attr__('edge_attr', torch.from_numpy(graph['edge_feat']),
                                  'edge_feature')
            del graph['edge_feat']

        if graph['node_feat'] is not None:
            g.__set_tensor_attr__('x', torch.from_numpy(graph['node_feat']),
                                  'node_feature')
            del graph['node_feat']

        for key in additional_node_files:
            g.__setattr__(key, torch.from_numpy(graph[key]))
            del graph[key]
